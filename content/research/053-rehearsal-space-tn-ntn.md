...
- an **orbital-dynamics twin** (ephemeris propagation, pass schedules, elevation-dependent beam geometry — the Ansys-STK class of tool);
- an **RF-propagation twin** (ray-traced, geometry-grounded city and terrain models — the class now industrialized by GPU tracers such as Sionna RT [30] and NVIDIA's Aerial Omniverse Digital Twin [29]);
- a **protocol-behavior twin** (3GPP mobility events, timers, conditional-handover logic per Rel-17/18 [13]);
- a **live-state twin** (the operator's counters, configurations and neighbor relations, streamed from the OSS — the layer operators like Rakuten Symphony have themselves argued for [35]).

The orchestration twin's job is the interfaces: at every simulation tick it hands the propagation twin a time-tagged beam footprint and EIRP mask from the orbital twin, hands the protocol twin the resulting per-location C/I, and reconciles all of it against live network state. O-RAN's research groups have mapped both the use cases and the enabling technologies for exactly this composition [33, 34]; what does not yet exist anywhere is the composed, operational whole.
...
The end-state that makes all of this worth building is not a better phone call. It is presence at a distance: holographic-type communication — volumetric, multi-sensory, eventually touchable. The requirements are on record. ITU's Network-2030 work names holographic-type communication a defining use case and quantifies the uncompressed extreme: raw holograms would demand up to terabits per second at sub-millisecond latency [47, 48] — a deliberate upper bound, echoed in industry 6G overviews [77], that compression exists to demolish. The IMT-2030 (6G) framework adopted "immersive communication" as one of six official usage scenarios [72]. Compression is closing the gap fast: MPEG's V-PCC reaches on the order of 100:1 on dynamic point clouds at usable quality operating points [50], codec avatars transmit semantics instead of pixels and already use less bandwidth than 2D video on shipping hardware [60, 64] — and the first commercial light-field telepresence products are on the market at $24,999 [63].
